...
- If any worker is still active on the same scope, keep waiting for that worker to finish instead of spinning up a replacement on the same work. If the worker ceiling is hit, wait for the active worker to finish or close an idle worker before continuing.
